<span class="text-sm text-gray-500 dark:text-gray-400 mb-4 block">
    {{ $jobs->total() }} listing{{ $jobs->total() !== 1 ? 's' : '' }}
    @if ($status !== 'all')
        <span class="text-{{ $status === 'pending' ? 'amber' : ($status === 'approved' ? 'green' : 'red') }}-500 font-medium">({{ $status }})</span>
    @endif
</span>
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Location</th>
        ...
        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-700">
            @forelse ($jobs as $job)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition relative z-0">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        @endphp
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium border {{ $statusClasses[$job->status] ?? 'bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700' }}">
                            @if ($job->status === 'approved')
                                <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>
                            @elseif ($job->status === 'pending')
                                <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            @else
                                <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                            @endif
                            {{ ucfirst($job->status) }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium ">
                        <div class="action-dropdown relative inline-block text-left">
                            ...
                            <div id="dropdown-{{ $job->id }}" class="action-dropdown-menu hidden absolute right-0 mt-2 w-52 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg z-50 origin-top-right">
                                <div class="py-1">
                                    @if ($job->status !== 'approved')
                                        <form method="POST" action="{{ route('admin.jobs.approve', $job) }}">
                                            @csrf @method('PATCH')
                                            <button type="submit"
                                               class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-green-700 dark:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/20 transition">
                                    ...
                                    @if ($job->status !== 'pending')
                                        <form method="POST" action="{{ route('admin.jobs.pending', $job) }}">
                                            @csrf @method('PATCH')
                                            <button type="submit"
                                               class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition">
                                    ...
                                    @if ($job->status === 'rejected' || $job->status === 'pending' || $job->status === 'approved')
                                        <div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                                    @endif

                                    <button type="button" onclick="openPreviewModal({{ $job->id }})"
                                       class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            ...
            @empty
                <tr>
                    <td colspan="9" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400">No jobs found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
